(ns jwplayer.main
  "Kotodama WASM entrypoint for the jwplayer clean-room actor (L4) — Clojure port.
  ...
  facts (`jwplayer.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "jwplayer")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"jwplayer.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "jwplayer-compat" :tier tier :entities entities} 200])
...
(defn start! [] :jwplayer-compat/ready)
